#let introduction = [
  = Introdução
  Com a ascensão das tecnologias de blockchain e o modelo de consenso de Prova de Trabalho (PoW), o cryptojacking consolidou-se como uma ameaça cibernética lucrativa, onde atacantes sequestram o poder de processamento de terceiros para minerar criptomoedas sem consentimento. O problema investigado no trabalho de referência @iotcryptojacking é a detecção dessa atividade maliciosa em redes domésticas inteligentes (IoT), ambientes que se tornaram alvos preferenciais devido à sua presença massiva, conectividade constante e vulnerabilidades críticas, como o uso de senhas padrão e a falta de autenticação robusta.
